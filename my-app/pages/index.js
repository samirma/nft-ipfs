...
export default function Home() {
  ...
  const fetchBalance = async () => {
    ...
    const rawBalance = await provider.getBalance(NFT_CONTRACT_ADDRESS);

    const value = parseFloat(ethers.utils.formatEther(rawBalance));
    // Only change balance state if it differs from previous one
  ...
  const withdraw = async () => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("You successfully withdrawed a LW3Punk!");
    } catch (err) {
      console.error(err);
    }
  };
